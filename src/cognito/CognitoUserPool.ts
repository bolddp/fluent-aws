...
export class CognitoUserPool extends AwsDataApiNode<UserPoolDescriptionType> {
  ...
  async signup(
    ...
    return await AwsApi.cognito(this.config()).signup(
      this.id.poolId,
      this.id.clientId,
      signupData.userName,
      signupData.password,
      attributeList,
      signupData.skipVerification
    );
  }
  ...
  async requestForgotPasswordCode(email: string): Promise<void> {
    await this.ensureResolved();
    await AwsApi.cognito(this.config()).forgotPassword(
      this.id.poolId,
      this.id.clientId,
      email
    );
  ...
  async setNewUserPassword(
    ...
    await this.ensureResolved();
    await AwsApi.cognito(this.config()).confirmPassword(
      this.id.poolId,
      this.id.clientId,
      email,
      verificationCode,
